function [startPosVec,endPosVec,seqVec] = findIR(fastaFile,minLength,maxLength)
...
% Inputs
%
% fastaFile     -   The name of the fasta file containing the input sequence, in char.
%                   The fasta file should contain only one sequence.
...
% Example:
%
%    [iRstartPos,iRendPos,iRSeq] = findIR('TAIR10_chr1.fas',4,1000)
...
fastaObj = fastaread(fastaFile);
seq = lower(fastaObj.Sequence);
...
if(iRCount > 0)
    fastaObj = fastaread(fastaFile);
    seq = fastaObj.Sequence;
    clear fastaObj;
    seqVec = cell(1,iRCount);
    for iRPos=1:iRCount
        seqVec{iRPos} = seq(startPosVec(iRPos):endPosVec(iRPos));
    end
else
    startPosVec = [];
    endPosVec = [];
